...
create_pdf ()
{
	...
	pandoc --from markdown --template /appdata/hhtemplate.tex --filter pandoc-tablenos --filter pandoc-fignos --filter pandoc-citeproc --filter pandoc-plantuml --pdf-engine=xelatex --listings --variable=hhdocumentfont:FreeSans --csl=/appdata/haaga-helia-university-of-applied-sciences-harvard.csl --verbose -o $FILEPDF $FILEMD --variable=hhtemplatetype:$TEMPLATETYPE || assert_ret $? 0
